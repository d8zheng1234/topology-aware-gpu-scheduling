...
This collector covers relationships between GPUs. Network interfaces are
...
The host-side view, which NUMA node owns a GPU and which interface is nearest
to it, is derived from both; see
[GPU, NUMA, and NIC host locality](host-topology.md). A typed affinity graph
and inter-node bandwidth and latency still require separate work, and
