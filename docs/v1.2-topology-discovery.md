...
Ray's task resource request selects a logical GPU on a chosen node, and Ray
chooses which physical device that is. [Device identity binding](device-binding.md)
now reads the actual CUDA-visible UUID, cross-checks the NVML candidate, and refuses a rank
that did not get the planned one, so a placement is honored or refused rather
than silently moved. That is verification, not selection: the adapter still
cannot ask Ray for a particular device or a particular NVLink edge.

Scoring on these edges would therefore still overstate the guarantee, and the
graph stays observational until selection exists. That work is tracked in
[issue #35](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/35).
...
GPU-to-NIC affinity and inter-node bandwidth and latency still require their
own work: affinity is not derived here, and advertised speed is not measured
throughput.
