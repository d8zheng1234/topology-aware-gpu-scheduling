...
The [typed topology graph](topology-graph.md) can load this GPU-only inventory
and represent additional NIC/NUMA observations without changing its existing
public models or discovery output. That API provides deterministic serialization
and affinity queries; it does not add automatic NIC or affinity discovery.
...
This collector covers intra-node GPU relationships. NIC location and speed,
GPU-to-NIC affinity, and inter-node bandwidth and latency require separate
collectors and measurements.

`TopologyGraph.from_observations()` can join this GPU inventory with exported
NIC and host-locality snapshots from those collectors, preserving their raw
evidence without changing NVML discovery. See the [snapshot adapter contract](topology-graph.md#adapting-collector-snapshots)
for node identity matching, unknown states, and conflict handling.
