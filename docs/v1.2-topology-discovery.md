...
This collector covers intra-node GPU relationships. NIC location and speed and
GPU-to-NIC affinity require separate collectors. Inter-node TCP bandwidth and
latency are measured separately and only on request; see
[inter-node link measurement](link-measurement.md).
