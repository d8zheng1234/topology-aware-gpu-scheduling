...
For controlled baseline wiring, `run_matched_trace()` repeats one ordered list
of `TraceJob` objects through this same discovery-input, planning, reservation,
execution, and cleanup path for all five policies. It records failures and
submission-to-terminal timing without retrying or changing backend options by
policy. See the [baseline guide](baseline-policies.md) for job boundaries,
GPU-count normalization, and caller-owned experimental controls. Run `python -m
examples.compare_policy_traces` for a two-node simulated integration check.
