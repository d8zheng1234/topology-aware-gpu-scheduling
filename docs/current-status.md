...
| Device identity binding | Implemented; constructed-device unit tests and a simulated-GPU Ray smoke | [Binding tests](../tests/test_device_binding.py), [Ray smoke](../examples/ray_device_binding_smoke.py), [guide](device-binding.md); verification only. Ray still selects the device, nothing has run on physical GPUs, and scoring does not use device-level edges. |
