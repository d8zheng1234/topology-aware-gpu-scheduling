...
| Inter-node TCP link measurement | Implemented; mocked unit tests, NIC collector fixtures, loopback socket tests, and a two-node Ray run on one host | [Link tests](../tests/test_links.py), [NIC integration tests](../tests/test_link_nic_evidence.py), [Ray link smoke](../examples/ray_link_smoke.py), [guide](link-measurement.md); endpoint provenance and missing identity stay explicit. Opt-in only; no physical multi-node measurement is recorded, and TCP results are not RDMA or NCCL throughput. |
