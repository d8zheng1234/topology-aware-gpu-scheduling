...
| V1.1 inventory and V1.2 intra-node topology | Physical one-GPU inventory validated; pair topology remains unverified | The [single-GPU report](one-gpu-validation.md) records real NVML identity, memory, and PCI discovery. [Inventory tests](../tests/test_inventory.py) cover pair topology with mocked NVML. No physical GPU pair was available, and graph edges remain observational rather than scoring inputs; a rank's device identity is checked separately, in the row below. |
| Device identity binding | Implemented; mocked CUDA/NVML tests and a simulated-GPU Ray smoke | [Binding tests](../tests/test_device_binding.py), [CUDA identity tests](../tests/test_cuda_identity.py), [Ray smoke](../examples/ray_device_binding_smoke.py), [GPU check](../examples/device_binding_gpu_check.py), [guide](device-binding.md); production checks the actual CUDA UUID before work. The historical Windows single-GPU report predates this verifier update; current physical multi-GPU and Linux assignment remain unverified. Ray still selects the device; tokens and observed edges do not enforce selection. |
