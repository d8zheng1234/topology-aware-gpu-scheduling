...
| Typed GPU/NIC/NUMA topology graph | Implemented; synthetic graph, snapshot fixtures, and NIC collector integration tests | [Graph API and boundaries](topology-graph.md), [graph tests](../tests/test_topology.py), [adapter tests](../tests/test_topology_observations.py); legacy GPU-only loading, collector snapshot joins, typed evidence, and categorical affinity queries; NIC collection exercised with in-memory sysfs, no physical affinity validation or device binding. |
| Automatic GPU/NUMA/NIC affinity and inter-node discovery | Planned | [NUMA/NIC mapping #15](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/15), [network measurements #17](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/17). The graph accepts collector observations but does not derive proximity or measure links. |
