...
| Typed GPU/NIC/NUMA topology graph | Implemented; synthetic graph and collector snapshot fixtures | [Graph API and boundaries](topology-graph.md), [graph tests](../tests/test_topology.py), [adapter tests](../tests/test_topology_observations.py); legacy GPU-only loading, collector snapshot joins, typed evidence, and categorical affinity queries; no device binding. |
| Automatic NIC/affinity and inter-node discovery | Planned | [NIC inventory #14](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/14), [NUMA/NIC mapping #15](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/15), [network measurements #17](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/17). The graph accepts observations but does not implement these collectors. |
