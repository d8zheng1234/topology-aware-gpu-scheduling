...
| Inter-node TCP link measurement | Implemented; mocked unit tests, loopback socket tests, and a two-node Ray run on one host | [Link tests](../tests/test_links.py), [Ray link smoke](../examples/ray_link_smoke.py), [guide](link-measurement.md); opt-in only. No physical multi-node measurement is recorded, and TCP results are not RDMA or NCCL throughput. |
| GPU-to-NIC affinity and NIC inventory | Planned | [NIC inventory #14](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/14), [NUMA/NIC mapping #15](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/15), [affinity graph #16](https://github.com/LawrenceL05/topology-aware-gpu-scheduling/issues/16). Link measurement resolves each probe's interface and advertised speed on Linux only as a stopgap. |
