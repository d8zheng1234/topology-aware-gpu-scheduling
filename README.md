...
- **[Device identity binding](topology_scheduler/device_binding.py)**: reserves one custom resource per physical GPU, resolves the device each rank received to its UUID, and refuses a rank that did not get the planned one; see the **[guide](docs/device-binding.md)**.
